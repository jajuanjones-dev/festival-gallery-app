...
export async function deletePreview(key) {
  await r2.send(new DeleteObjectCommand({ Bucket: PREVIEWS_BUCKET, Key: key }));
}

export async function deleteEnhanced(key) {
  await r2.send(new DeleteObjectCommand({ Bucket: ORIGINALS_BUCKET, Key: key }));
}
