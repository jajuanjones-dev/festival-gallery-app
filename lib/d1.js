...
export async function insertPhoto({ id, festivalId, timestamp, previewKey, enhancedKey }) {
  await d1Query(
    "INSERT INTO photos (id, festival_id, timestamp, preview_key, enhanced_key, original_key, purchased) VALUES (?, ?, ?, ?, ?, ?, 0)",
    [id, festivalId, timestamp, previewKey, enhancedKey, enhancedKey]
  );
}
...
export async function createEvent(id, displayName) {
  const createdAt = new Date().toISOString();
  await d1Query(
    "INSERT INTO events (id, display_name, status, created_at) VALUES (?, ?, 'active', ?)",
    [id, displayName, createdAt]
  );
  return { id, display_name: displayName, status: "active", created_at: createdAt };
}
...
export async function updateEventDate(id, eventDate) {
  await d1Query("UPDATE events SET event_date = ? WHERE id = ?", [eventDate, id]);
}
